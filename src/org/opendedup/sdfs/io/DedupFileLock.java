 ...
 * 
 * @author annesam
 * Lock Object for DedupFiles. Locks do not actually prevent writing to locked space but rather reserve space so that other write
...
public class DedupFileLock {
	 ...
	 * @param ch DedupFileChannel associated with this lock.
	 * @param position the position where the lock start position is located.
	 * @param len the length of the lock
	 * @param shared if the lock is shared for reading or not.
	 */
	public DedupFileLock(DedupFileChannel ch,long position, long len, boolean shared) {
		this.channel = ch;
	 ...
	 * @param pos the proposed position of the lock
	 * @param sz the size of the proposed lock
