...
public class ReadAhead implements Runnable {
	...
	public static ReadAhead getReadAhead(SparseDedupFile df)
		...
		public void run() {
			try {
				...
				SDFSLogger.getLog().debug(
						"error caching chunk [" + buf.getFilePosition()
								+ "] in " + df.getDatabasePath(), e);
	...
	public void run() {
		...
		while (i < df.mf.length()) {
			try {
				WritableCacheBuffer buf = (WritableCacheBuffer) df
						.getWriteBuffer(i);
				...
				SDFSLogger.getLog().debug(
						"error caching chunk [" + i + "] in "
								+ df.getDatabasePath(), e);
				break;
