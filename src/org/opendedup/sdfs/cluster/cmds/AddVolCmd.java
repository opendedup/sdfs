	...
	public void executeCmd(final ClusterSocket soc) throws IOException {
		...
		try {
			...
			for(Rsp<Object> rsp : lst) {
				if(rsp.hasException()) {
					SDFSLogger.getLog().error("Add Volume to Cache Exception thrown for " + rsp.getSender());
					throw rsp.getException();
				} else if(rsp.wasSuspected() | rsp.wasUnreachable()) {
					SDFSLogger.getLog().error("Add Volume to Cache Host unreachable for " + rsp.getSender());
				}
