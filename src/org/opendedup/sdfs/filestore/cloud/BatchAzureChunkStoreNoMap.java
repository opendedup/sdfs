 ...
 * 
 */
public class BatchAzureChunkStoreNoMap implements AbstractChunkStore,
	...
	public BatchAzureChunkStoreNoMap() {
							...
							if (objs <= delobj) {
								...
								int compressedSize = Integer
										.parseInt((String) metaData
												.get("compressedsize"));
								if (HashBlobArchiveNoMap.compressedLength.get() > 0) {

									HashBlobArchiveNoMap.compressedLength.addAndGet(-1
											* compressedSize);
								} else if (HashBlobArchiveNoMap.compressedLength.get() < 0)
									HashBlobArchiveNoMap.compressedLength.set(0);
								HashBlobArchiveNoMap.currentLength.addAndGet(-1 * size);
								if (HashBlobArchiveNoMap.currentLength.get() > 0) {
									HashBlobArchiveNoMap.currentLength.addAndGet(-1 * size);
								} else if (HashBlobArchiveNoMap.currentLength.get() < 0)
									HashBlobArchiveNoMap.currentLength.set(0);
	...
	public StringResult getStringResult(String key) throws IOException,
		...
		try {
			...
			HashBlobArchiveNoMap.currentLength.addAndGet(_sz);
			HashBlobArchiveNoMap.compressedLength.addAndGet(_cl);
		} catch (Exception e) {
