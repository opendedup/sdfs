...
public class DownloadShard implements Runnable {
	...
	public DownloadShard(AbstractDownloadListener l,String name,int pos,int len,AmazonS3Client s3Service,String bucket) {
		b = new byte[len];
		this.l = l;
	...
	public void run() {
		try {
			...
			//SDFSLogger.getLog().debug("unable to read " + obj.getKey() + " at pos=" + pos + " byte len=" + b.length + " file size=" +obj.getObjectMetadata().getContentLength(),e);
			l.commandException(e);
