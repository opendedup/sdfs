...
public class ProcessVolumeInfo {
	public static void runCmd() {
		try {
			...
			Element dse = (Element) root.getElementsByTagName("volume").item(0);
			...
			long dedupSz = Long.parseLong(dse.getAttribute("duplicate-bytes"));
			long aWriteBites = Long.parseLong(dse.getAttribute("write-bytes"));
			long readBytes = Long.parseLong(dse.getAttribute("read-bytes"));
			...
			System.out.printf("Volume Unique Data Written: %s\n", StorageUnit
					.of(aWriteBites).format(aWriteBites));
			System.out.printf("Volume Data Read : %s\n",
					StorageUnit.of(readBytes).format(readBytes));
			if (dedupSz == 0 || aWriteBites == 0) {
				System.out.printf("Volume Virtual Dedup Rate (Dup/Total Bytes Written) : %d%%\n", 0);
			} else {
				double dedupRate = (((double) dedupSz / (double) totalBytes) * 100);
				DecimalFormat twoDForm = new DecimalFormat("#.##");
				dedupRate = Double.valueOf(twoDForm.format(dedupRate));
				System.out.printf("Volume Virtual Dedup Rate (Dup/Total Bytes Written) : %s%%\n",
						Double.toString(dedupRate));
			}
			if (totalBytes ==0 || dseSz==0) {
				System.out.printf("Volume Real Dedup Rate (DSE Size/Total Bytes Written) : %d%%\n", 0);
			} else {
				double dedupRate = (1-((double) dseSz / (double) totalBytes)) * 100;
				DecimalFormat twoDForm = new DecimalFormat("#.##");
				dedupRate = Double.valueOf(twoDForm.format(dedupRate));
				System.out.printf("Volume Real Dedup Rate (DSE Size/Total Bytes Written) : %s%%\n",
						Double.toString(dedupRate));
			}
			if (currentSz == 0 || aWriteBites == 0) {
				System.out.printf("Actual Storage Savings (Unique Blocks Stored/Current Size): %d%%\n", 0);
			} else {
				double dedupRate = (1-((double) dseSz / (double)currentSz )) * 100;
				DecimalFormat twoDForm = new DecimalFormat("#.##");
				dedupRate = Double.valueOf(twoDForm.format(dedupRate));
				System.out.printf("Volume Actual Storage Savings (Unique Blocks Stored/Current Size) : %s%%\n",
						Double.toString(dedupRate));
			}
		} catch (Exception e) {
