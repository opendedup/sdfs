...
public class MgmtServerConnection {
	...
	static {
		...
		String target = server + ":" + gRPCPort;
		try {
			channel = ManagedChannelBuilder.forTarget(target).usePlaintext().build();
		} catch (Exception e) {
			if (e.getCause() instanceof java.net.ConnectException) {
				System.err.println("Unable to connect to server " + target);
			} else {
				e.printStackTrace();
			}
		}
	...
	public static String initAuth(String password, String svr, int pt, boolean ssl) throws IOException {
		...
		try {
			...
			if (returnCode != 200)
				throw new IOException("Unable to process command " + method.getQueryString() + " return code was"
						+ returnCode + " return msg was " + method.getResponseBodyAsString());
			in = method.getResponseBodyAsStream();
			Document doc = db.parse(in);
			Element el = doc.getDocumentElement();
			String salt = el.getAttribute("salt");
			sessionId = el.getAttribute("session-id");
			String key = HashFunctions.getSHAHash(password.trim().getBytes(), salt.getBytes());
		...
		try {
			...
			if (userName != null && hmac != null)
				if (_url.trim().length() == 0)
					_url = "username=" + URLEncoder.encode(userName, "UTF-8") + "&hmac="
							+ URLEncoder.encode(hmac, "UTF-8") + "&timestamp=" + ts;
				else
					_url = _url + "&username=" + URLEncoder.encode(userName, "UTF-8") + "&hmac="
							+ URLEncoder.encode(hmac, "UTF-8") + "&timestamp=" + ts;
			return _url;
		} catch (Exception e) {
			throw new IOException(e);
	...
	public static GetMethod connectAndGet(String url, String file) throws Exception {
		return connectAndGet(url, file, useSSL);
	}

	public static GetMethod connectAndGet(String url, String file, boolean useSSL) throws Exception {
		url = createAuthUrl(url, baseHmac);
		String prot = "http";
		if (useSSL) {
			prot = "https";
	...
	public static GetMethod connectAndGet(String server, int port, String password, String url, String file,
			boolean useSSL) throws Exception {
		String req = null;
		try {
			if (userName != null && password != null)
				if (url.trim().length() == 0)
					url = "username=" + URLEncoder.encode(userName, "UTF-8") + "&password="
							+ URLEncoder.encode(password, "UTF-8");
				else
					url = url + "&username=" + URLEncoder.encode(userName, "UTF-8") + "&password="
							+ URLEncoder.encode(password, "UTF-8");
			String prot = "http";
			if (useSSL) {
				prot = "https";
	...
	public static GetMethod connectAndGetHMAC(String server, int port, String url, String file, boolean useSSL)
			throws Exception {
		String req = null;
		try {
	...
	public static PostMethod connectAndPost(String server, int port, String url, String file, String postData,
			boolean useSSL) throws Exception {
		String req = null;
		try {
			String prot = "http";
	...
	public static void main(String[] args) throws IOException {
		...
		MgmtServerConnection.baseHmac = MgmtServerConnection.initAuth("admin", MgmtServerConnection.server,
