...
public final class LongProductionMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<
  ...
  public enum EvictionPolicy {
    ...
    FIFO() {
      @Override
      <K, V> void onAccess(LongProductionMap<K, V> map, Node<K, V> node) {
        // do nothing
      }

      @Override
      <K, V> boolean onEvict(LongProductionMap<K, V> map, Node<K, V> node) {
        return true;
      }
    },
     ...
     */
    abstract <K, V> void onAccess(LongProductionMap<K, V> map, Node<K, V> node);

    /**
     * Determines whether to evict the node at the head of the list.
     */
    abstract <K, V> boolean onEvict(LongProductionMap<K, V> map, Node<K, V> node);
  }
  ...
  private final class KeySet extends AbstractSet<K> {
    private final LongProductionMap<K, V> map = LongProductionMap.this;
  ...
  private final class KeyIterator implements Iterator<K> {
    private final EntryIterator iterator =
        new EntryIterator(LongProductionMap.this.data.values().iterator());
  ...
  private final class EntryIterator implements Iterator<Entry<K, V>> {
    ...
    public void remove() {
      ...
      LongProductionMap.this.remove(current.getKey(), current.getValue());
      current = null;
    }
  }
