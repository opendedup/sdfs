...
public class ShardedProgressiveFileBasedCSMap3 implements AbstractMap, AbstractHashesMap {
	...
	private ShardedFileByteArrayLongMap3 activeWMap = null;
	...
	private ShardedFileByteArrayLongMap3 createWriteMap() throws IOException {
		ShardedFileByteArrayLongMap3 activeWMap = null;
		try {
			String guid = null;
			...
			while (!written) {
				...
				if (!f.exists()) {
					activeWMap = new ShardedFileByteArrayLongMap3(fileName + "-" + guid, this.hashTblSz);
					activeWMap.activate();
					activeWMap.setUp();
	...
	private ShardedFileByteArrayLongMap3 getWriteMap() throws IOException {
			...
			while (iter.hasNext()) {
				...
				try {
					...
					/*
						...
						KVPair p = m.nextKeyValue();
						while (p != null) {
							ShardedFileByteArrayLongMap3 _m = this.getWriteMap();
							try {
								_m.put(p.key, p.value, p.loc);
								p = m.nextKeyValue();
							} catch (HashtableFullException e) {
	...
	public long setUp() throws Exception {
		...
		if (files.length > 0) {
			...
			for (int i = 0; i < files.length; i++) {
				...
				ShardedFileByteArrayLongMap3 m = null;
				String pth = files[i].getPath();
				String pfx = pth.substring(0, pth.length() - 5);
				m = new ShardedFileByteArrayLongMap3(pfx, sz);
				long mep = m.setUp();
				if (mep > endPos)
					endPos = mep;
				maps.add(m);
	...
	public static void main(String[] args) throws Exception {
		ShardedProgressiveFileBasedCSMap3 b = new ShardedProgressiveFileBasedCSMap3();
