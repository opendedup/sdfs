...
public class ProgressiveFileBasedCSMap implements AbstractMap, AbstractHashesMap {
	...
	private int hashTblSz = 1250000;
	private ArrayBlockingQueue<ProgressiveFileByteArrayLongMap> activeReadMaps = new ArrayBlockingQueue<ProgressiveFileByteArrayLongMap>(
			Main.writeThreads);
	...
	private AbstractShard getReadMap(byte[] hash) throws IOException {
		...
		while (iter.hasNext()) {
			...
			if (_m.containsKey(hash)) {
				al.lock();
				try {
					if (!activeReadMaps.contains(_m)) {
						// SDFSLogger.getLog().info("adding active " +
						// _m.toString());
						if (activeReadMaps.remainingCapacity() == 0) {
							ProgressiveFileByteArrayLongMap obf = activeReadMaps
									.poll();
			...
			while (!written) {
				...
				if (!f.exists()) {
					activeWMap = new ProgressiveFileByteArrayLongMap(fileName + "-"
							+ guid, NextPrime.getNextPrimeI(this.hashTblSz));
					activeWMap.setUp();
					this.maps.add(activeWMap);
					activeWMap.active = true;
					written = true;
				}
			}
			return activeWMap;
	...
	public long setUp() throws Exception {
		...
		SDFSLogger.getLog().debug(
				"maxsz=" + this.maxSz + " _tbs=" + _tbs + " hashTblSz="
						+ this.hashTblSz);
		...
		if (maps.size() == 0)
			...
			try {
				lbf = new LargeBloomFilter(_fs.getParentFile(), maxSz, .03,
						true);
			} catch (Exception e) {
			...
			while (!written) {
				...
				if (!f.exists()) {
					ProgressiveFileByteArrayLongMap activeWMap = new ProgressiveFileByteArrayLongMap(
							fileName + "-" + guid,
							NextPrime.getNextPrimeI(this.hashTblSz));
					activeWMap.setUp();
					this.maps.add(activeWMap);
					written = true;
					activeWMap.active = true;
	...
	public boolean put(ChunkData cm, boolean persist) throws IOException,
		...
		try {
			if (lbf.mightContain(cm.getHash())) {
