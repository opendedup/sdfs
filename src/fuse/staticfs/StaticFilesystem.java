...
public class StaticFilesystem implements Filesystem1 {
	...
	public FuseDirEnt[] getdir(String path) throws FuseException {
		...
		for (int i = 0; i < children.length; i++) {
			...
			FuseStat stat = (child instanceof MountpointNode) ? ((MountpointNode) child)
					.getFilesystem().getattr("/")
					: child.getStat();
	...
	public void link(String from, String to) throws FuseException {
		...
		if (fromIsMount || toIsMount) {
			if (fromIsMount && toIsMount && fromRr.node == toRr.node) {
				((MountpointNode) fromRr.node).getFilesystem().link(
						fromRr.path, toRr.path);
			} else {
				throw new FuseException("Cross Device Link not possible")
						.initErrno(FuseException.EXDEV);
			}
		} else {
			throw new FuseException("Read Only").initErrno(FuseException.EROFS);
		}
	...
	public void mknod(String path, int mode, int rdev) throws FuseException {
		...
		if (rr.node instanceof MountpointNode) {
			((MountpointNode) rr.node).getFilesystem().mknod(rr.path, mode,
					rdev);
			return;
		}
	...
	public void read(String path, ByteBuffer buf, long offset)
		...
		if (rr.node instanceof MountpointNode) {
			((MountpointNode) rr.node).getFilesystem().read(rr.path, buf,
					offset);
			return;
		}
	...
	public String readlink(String path) throws FuseException {
		...
		throw new FuseException("Not a Symbolic Link")
				.initErrno(FuseException.EINVAL);
	}
	...
	public void rename(String from, String to) throws FuseException {
		...
		if (fromIsMount || toIsMount) {
			if (fromIsMount && toIsMount && fromRr.node == toRr.node) {
				((MountpointNode) fromRr.node).getFilesystem().rename(
						fromRr.path, toRr.path);
			} else {
				throw new FuseException("Cross Device Rename not possible")
						.initErrno(FuseException.EXDEV);
			}
		} else {
			throw new FuseException("Read Only").initErrno(FuseException.EROFS);
	...
	public void utime(String path, int atime, int mtime) throws FuseException {
		...
		if (rr.node instanceof MountpointNode) {
			((MountpointNode) rr.node).getFilesystem().utime(rr.path, atime,
					mtime);
			return;
		}
	...
	public void write(String path, ByteBuffer buf, long offset)
		...
		if (rr.node instanceof MountpointNode) {
			((MountpointNode) rr.node).getFilesystem().write(rr.path, buf,
					offset);
			return;
		}
	...
	private ResolveResult resolvePath(String path, boolean resolveParent)
			throws FuseException {
		Node node = rootNode;
		int i = 0;
		int subPathStart = 0;
		...
		while (i < path.length() && !(node instanceof MountpointNode)) {
			...
			// are we resolving parent node & have just parsed the last
			// component of path?
			if (resolveParent && i >= path.length())
				break;
			...
			if (name.equals(".")) {
				...
				if (node instanceof DirectoryNode
						&& (childNode = ((DirectoryNode) node).getChild(name)) != null)
					node = childNode;
				else
