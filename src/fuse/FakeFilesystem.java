...
public class FakeFilesystem implements Filesystem3, XattrSupport {
	...
	private N lookup(String path) {
		...
		N node = (parent instanceof D) ? ((D) parent).files.get(f.getName())
				: null;
	...
	public FakeFilesystem() {
		...
		root.add(new F("execute_me.sh", 0755,
				"#!/bin/sh\n\necho \"You executed me\"\n", "mimetype",
				"text/plain", "description", "a BASH script"));

		D subdir = new D("subdir", 0755, "description", "a subdirectory");
		root.add(subdir);
		subdir.add(new L("README.link", 0666, "../README", "description",
				"a symbolic link"));
		subdir.add(new L("execute_me.link.sh", 0666, "../execute_me.sh",
	...
	public int chmod(String path, int mode) throws FuseException {
		...
		if (n != null) {
			n.mode = (n.mode & FuseStatConstants.TYPE_MASK)
	...
	public int getattr(String path, FuseGetattrSetter getattrSetter)
		...
		if (n instanceof D) {
			...
			L l = (L) n;
			getattrSetter.set(l.hashCode(), FuseFtypeConstants.TYPE_SYMLINK
					| l.mode, 1, 0, 0, 0, l.link.length(), (l.link.length()
					+ BLOCK_SIZE - 1)
					/ BLOCK_SIZE, time, time, time);
			...
			for (N child : ((D) n).files.values()) {
				int ftype = (child instanceof D) ? FuseFtypeConstants.TYPE_DIR
						: ((child instanceof F) ? FuseFtypeConstants.TYPE_FILE
								: ((child instanceof L) ? FuseFtypeConstants.TYPE_SYMLINK
										: 0));
				if (ftype > 0)
					filler
	...
	public int read(String path, Object fh, ByteBuffer buf, long offset)
		...
		if (fh instanceof FH) {
			F f = (F) ((FH) fh).n;
			buf.put(f.content, (int) offset, Math.min(buf.remaining(),
					f.content.length - (int) offset));
