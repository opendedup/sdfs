...
public class ChunkEncoder {
	...
	/**
	 * @param totalLength
	 *            Total encoded length; used for calculating size of hash table
	 *            to use
	 */
	...
	public LZFChunk encodeChunk(byte[] data, int offset, int len) {
		if (len >= MIN_BLOCK_TO_COMPRESS) {
			...
			if (compLen < (len - 2)) { // nah; just return uncompressed
				return LZFChunk
						.createCompressed(len, _encodeBuffer, 0, compLen);
			}
		}
	...
	private int tryCompress(byte[] in, int inPos, int inEnd, byte[] out,
			int outPos) {
		int literals = 0;
		outPos++;
		int hash = first(in, 0);
		...
		while (inPos < inEnd) {
			...
			if (ref < inPos && ref >= firstPos
					&& (off = inPos - ref - 1) < MAX_OFF && in[ref + 2] == p2
