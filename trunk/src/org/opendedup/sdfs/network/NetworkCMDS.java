 ...
 * These are the commands that are sent by the client to the chunk store. The command is sent as the 
 * first byte in a command request. A typical client request is as follows :
 * 
 *   |command type (1b)|length of hash (2b)|md5 or sha hash (lenghth of hash)| command specific data (variable length)|
 *
 */
...
public class NetworkCMDS {
	...
	/** Write a compressed chunk to the chunk server. The data will be compressed by the client before it is sent.*/
