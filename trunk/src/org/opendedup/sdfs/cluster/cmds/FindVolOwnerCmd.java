	...
	public void executeCmd(final ClusterSocket soc) throws IOException {
		...
		try {
			...
			for(Rsp<Object> rsp : lst) {
				if(rsp.hasException()) {
					SDFSLogger.getLog().error("FIND_VOLUME_OWNER Exception thrown for " + rsp.getSender());
				} else if(rsp.wasSuspected() | rsp.wasUnreachable()) {
					SDFSLogger.getLog().error("FIND_VOLUME_OWNER Host unreachable for " + rsp.getSender());
				}
				else if(rsp.getValue() != null) {
					SDFSLogger.getLog().debug("FIND_VOLUME_OWNER completed for " +rsp.getSender() + " returned=" +rsp.getValue());
					boolean m = (Boolean)rsp.getValue();
					if(m) {
						if(this.owner != null)
							throw new IOException("VOLUME_OWNER already identified at [" +owner.toString() + "] but has also been identified at [" +rsp.getSender()+"].");
						else
