...
public class StopGCMasterCmd implements IOPeerCmd {
	...
	public void executeCmd(final ClusterSocket soc) throws IOException {
		...
		try {
					...
					if(rsp.getValue() != null) {
						...
						if(m) {
							if(this.gcMaster != null)
								throw new IOException("STOP_GC_MASTER_CMD already identified at [" +gcMaster.toString() + "] but has also been identified at [" +rsp.getSender()+"].");
							else
			...
			SDFSLogger.getLog().error("error while running STOP_GC_MASTER_CMD", e);
