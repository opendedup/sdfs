...
public class SetEnablePerfMonCmd implements XtendedCmd {
	...
	public String getResult(String cmd, String file) throws IOException {
		try {
			...
			return "SUCCESS Dedup Success: set dedup to [" + Main.volume.getName()
					+ "]  [" + dedup + "]";
		} catch (Exception e) {
			SDFSLogger.getLog().error(
					"ERROR Perfmon in perfmon event: unable to set perfmon on ["
							+  Main.volume.getName() + "] " + "to [" + cmd + "] because :"
							+ e.getMessage(), e);
			SDFSEvent.perfMonEvent("Disabled performance montitor").endEvent("ERROR Perfmon in perfmon event: unable to set perfmon on ["
					+  Main.volume.getName() + "] " + "to [" + cmd + "] because :"
					+ e.getMessage());
