...
*/
public class VariableSparseDedupFile implements DedupFile {
	...
	*/		
	...
	public VariableSparseDedupFile(MetaDataDedupFile mf) throws IOException {
	...
	public DedupFile snapshot(MetaDataDedupFile snapmf) throws IOException,
		...
		VariableSparseDedupFile _df = null;
		try {
			ch = this.getChannel(-1);
			this.writeCache();
			this.sync();
			_df = new VariableSparseDedupFile(snapmf);
	...
	public DedupChunkInterface getWriteBuffer(long position, boolean newBuff)
		...
		try {
			...
			this.writeBufferLock.lock();
			DedupChunkInterface writeBuffer = (DedupChunkInterface) this.writeBuffers
					.get(chunkPos);
			if (writeBuffer == null) {
	 ...
	 */
	@Override
	public DedupChunkInterface getReadBuffer(long position) throws FileClosedException,
			IOException {
		if (this.closed) {
			throw new FileClosedException("file already closed");
		}
		...
		while (l > -1) {
			try {
				...
				if (pck.isLocalData() && mf.isDedup()) {
					...
					DedupChunkInterface ck = new DedupChunk(pck.getHash(), chunk, l,
							Main.CHUNK_LENGTH);
					writeBuffer = new WritableCacheBuffer(ck, this);
	...
	private DedupChunkInterface createNewChunk(long location) {
		DedupChunkInterface ck = new DedupChunk(new byte[HashFunctionPool.hashLength],
