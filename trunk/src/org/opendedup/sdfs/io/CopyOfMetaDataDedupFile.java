 ...
 */
public class CopyOfMetaDataDedupFile implements Serializable {
	...
	public String getXAttribute(String name) {
		log.info("looking for extended attr " + name + " "
				+ this.extendedAttrs.size());
   ...
   public void setVmdk(boolean vmdk) {
	...
	public CopyOfMetaDataDedupFile() {
	}
	 ...
	 */
	public CopyOfMetaDataDedupFile(String path) {
		init(path);
	}
	 ...
	 *            the file name
	 */
	public CopyOfMetaDataDedupFile(String parent, String child) {
	 ...
	 *            the file name
	 */
	public CopyOfMetaDataDedupFile(File parent, String child) {
	 ...
	 * @throws IOException
	 */
	public CopyOfMetaDataDedupFile snapshot(String snaptoPath, boolean overwrite)
			throws IOException {
		if (!this.isDirectory()) {
			...
			CopyOfMetaDataDedupFile _mf = new CopyOfMetaDataDedupFile(snaptoPath);
			...
			_mf.lastAccessed = System.currentTimeMillis();
			...
			CopyOfMetaDataDedupFile[] files = this.listFiles();
			for (int i = 0; i < files.length; i++) {
				CopyOfMetaDataDedupFile file = files[i];
	...
	protected void mashal() {
		try {
			CopyOfMetaDataDedupFile df = MetaFileStore
			...
			if (df != null) {
				...
				this.vmdk = df.vmdk;
				this.dedup = df.dedup;
				this.lastAccessed = df.lastAccessed;
	 ...
	 *         directory
	 */
	public CopyOfMetaDataDedupFile[] listFiles() {
		File f = new File(this.path);
		if (f.isDirectory()) {
			String[] files = f.list();
			CopyOfMetaDataDedupFile[] df = new CopyOfMetaDataDedupFile[files.length];
	...
	public boolean renameTo(CopyOfMetaDataDedupFile dest) {
		File f = new File(this.path);
		if (f.isDirectory()) {
