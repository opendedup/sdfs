...
public class PFullGC implements GCControllerImpl {
	...
	public PFullGC() {
		...
		SDFSLogger.getLog().info("Current DSE Percentage Full is [" + this.prevPFull + "] will run GC when [" +this.nextPFull + "]");
	}
	...
	private double calcPFull() {
		...
		if(HashChunkService.getSize() > 0) {
			pFull = (double)HashChunkService.getSize()/(double)HashChunkService.getMaxSize();
		}
		return pFull;
	}
