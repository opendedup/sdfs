...
public class DSECompaction {
	public static synchronized void runCheck(AbstractHashesMap map,
			AbstractChunkStore nstore, AbstractChunkStore ostore)
			throws IOException {
		...
		try {
			...
			CommandLineProgressBar bar = new CommandLineProgressBar(
					"Scanning DSE", ostore.size() / Main.CHUNK_LENGTH,
					System.out);
			...
			System.out.println("Succesfully Ran Compaction for [" + records
					+ "] records out of [" + currentCount + "]");
			SDFSLogger.getLog().warn(
					"Succesfully Ran Compaction for [" + records + "] records");
			evt.endEvent("Succesfully Ran Compaction for [" + records
					+ "] records");
		} catch (Exception e) {
			SDFSLogger.getLog().error("Unable to finish compaction because", e);
			System.err.println("Unable to finish compaction because"
					+ e.toString());
	...
	public static synchronized void runCheck(AbstractHashesMap map,
		...
		try {
			...
			SDFSLogger
					.infoConsoleMsg("Step 3 of 3 - Committing FileStore Changes");
		...
		SDFSLogger
				.infoConsoleMsg("Finished Compaction - Commited FileStore Changes");
