 ...
 * <p/>
 * This is necessary in order to map remote users to local user when the remote
 * system has different uid's to the local system or when the remote system does
 * not provide uid's.
 */
...
public class PasswordEntry extends Struct {
	...
	public PasswordEntry(Charset cs, ByteBuffer username, int uid, int gid,
			ByteBuffer homeDirectory, ByteBuffer shell) {
	 ...
	 * 
	 * @param cs
	 *            character set used to map strings.
	 * @param username
	 *            the username to lookup
	 * @return the password entry or null if an error (not found) occurs.
	 */
	public static native PasswordEntry lookupByUsername(Charset cs,
			String username);
