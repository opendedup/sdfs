...
public class SparseDataChunk {
	...
	public SparseDataChunk(byte [] rawData) throws IOException {
		if(rawData.length != 1+Main.hashLength +1+8)
			throw new IOException("possible data corruption: byte array length " +rawData.length + " does not equal " + (1+Main.hashLength +1+8));
	...
	public SparseDataChunk(boolean doop, byte[] hash, boolean localData, long timeAdded) {
		this.doop = doop;
		this.hash = hash;
